    ...
    >
      ...
      <main className="max-w-5xl mx-auto px-3 sm:px-5 lg:px-7 xl:px-10 2xl:px-14 py-7 lg:py-10">
        <div className="space-y-5 lg:space-y-7">
          ...
          <div className={`bg-white/90 rounded-3xl border-4 border-white p-4 sm:p-6 lg:p-10 shadow-lg overflow-hidden ${
            trialMode === 'single' ? 'h-[300px]' : 'h-[600px]'
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  <img 
                    src={loadingGif} 
                    alt="Loading..." 
                    className="h-20 w-20 sm:h-28 sm:w-28 lg:h-36 lg:w-36 mx-auto mb-5"
                  />
                  <p className="text-base sm:text-lg lg:text-xl text-gray-600">Loading...</p>
                </div>
              </div>
            )}

            {results && !isRunning && (
              <div className="flex flex-col space-y-3 sm:space-y-4 lg:space-y-5 h-full overflow-y-auto">
                {trialMode === 'single' ? (
                  // Single Trial Result
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      <div className={`inline-flex items-center justify-center w-20 h-20 sm:w-28 sm:h-28 lg:w-36 lg:h-36 rounded-full mb-4 sm:mb-5 ${
                  ...
                  <>
                    {/* Summary Stats */}
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 lg:gap-5 text-center flex-shrink-0">
                      <div className="bg-green-100 rounded-lg p-2 sm:p-3">
                        <h3 className="text-lg sm:text-xl font-bold text-green-600">{results.successes}</h3>
                        <p className="text-green-700 text-xs sm:text-sm">Successes</p>
                        <p className="text-xs text-green-600">
                          {((results.successes / effectiveNumFlips) * 100).toFixed(1)}%
                        </p>
                      </div>
                      <div className="bg-red-100 rounded-lg p-2 sm:p-3">
                        <h3 className="text-lg sm:text-xl font-bold text-red-600">{results.fails}</h3>
                        <p className="text-red-700 text-xs sm:text-sm">Fails</p>
                        <p className="text-xs text-red-600">
                          {((results.fails / effectiveNumFlips) * 100).toFixed(1)}%
                        </p>
                      </div>
                      <div className="bg-blue-100 rounded-lg p-2 sm:p-3">
                        <h3 className="text-lg sm:text-xl font-bold text-blue-600">{effectiveNumFlips}</h3>
                        <p className="text-blue-700 text-xs sm:text-sm">Total Trials</p>
                    ...
                    <div className="flex flex-col flex-1 min-h-0">
                      <h3 className="text-base sm:text-lg font-bold text-gray-700 mb-2 sm:mb-3 flex-shrink-0">Flip Results:</h3>
                      <div className="flex-1 overflow-y-auto bg-gray-50 rounded-lg p-2 sm:p-3">
                        <div className="grid grid-cols-8 sm:grid-cols-12 md:grid-cols-16 lg:grid-cols-20 xl:grid-cols-24 gap-1 sm:gap-1.5">
                          {results.flips.map((flip, index) => (
                            <div
                              key={index}
                              className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ${
          ...
          <div className="flex justify-center">
            <div className="flex flex-col items-center gap-2">
              <button 
                ...
                className="bg-gradient-to-r from-yellow-100 to-orange-200 rounded-2xl border-3 border-yellow-200 px-4 sm:px-5 py-2 shadow-lg hover:from-yellow-200 hover:to-orange-300 hover:scale-105 transform transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
              >
                <span className="text-sm sm:text-base font-bold text-orange-700 flex items-center gap-2">
